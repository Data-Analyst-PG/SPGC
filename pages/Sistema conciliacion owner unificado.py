...
Version: 3.0 - Optimizada y Modular

Características principales:
- Procesamiento paralelo para mejor rendimiento
- Sistema de scoring inteligente para matching
- Manejo robusto de conceptos multiidioma
...
class PreparadorDatos:
    ...
    def preparar_contabilidad(self, 
        ...
        Args:
            df: DataFrame de contabilidad
            tipo_mov: Filtrar por tipo de movimiento ('D', 'H', o None para todos)
        
        Returns:
            Tuple[DataFrame preparado, mapa de columnas usadas]
        """
        out = df.copy()
        
        # Resolver nombres de columnas (flexibilidad en nombres)
        col_map = {
            ...
            'unidad': self._resolver_col(df, ['Unidad', 'Numero de Unidad', 'NumeroUnidad']),
    ...
    def preparar_cheques(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        Returns:
            Tuple[cheques_validos, cheques_excluidos]
        """
        col_map = {
    ...
    def preparar_vouchers(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        Returns:
            Tuple[vouchers_validos, vouchers_excluidos]
        """
        col_map = {
    ...
    def _elegir_col_importe(self, df: pd.DataFrame) -> str:
        ...
        Si hay duplicados (Importe, Importe.1), elige el último.
        """
        ...
        # Retornar el último (normalmente el del movimiento)
...
class MotorMatching:
    ...
    def match_base_vs_contabilidad(self, 
        ...
        Criterios:
        1. POLIZA_KEY (contrarecibo vs clavepoliza)
        2. UNIDAD_KEY
        3. VIAJE_KEY (referencia)
        4. CONCEPTO_KEY (flexible)
        5. IMPORTE_KEY
        
        Returns:
            Tuple[base_clasificada, contabilidad_clasificada, detalle_matches]
        """
        st.info(f"🔍 Matching Base Saldos ({len(base):,}) vs Contabilidad ({len(contabilidad):,})...")
        
        # Crear índices para búsqueda rápida
        cont_indexed = contabilidad.set_index('ROW_ID_CONT')
        ...
        # Clasificar contabilidad (vista inversa)
    ...
    def match_costos_vs_contabilidad(self, 
                                    ...
                                    contabilidad: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """
        Match de Costos (Cheques+Vouchers depurados) vs Contabilidad D
        
        Criterios (considerando que Contrarecibo puede ser NULL):
        1. VALE_KEY
        2. UNIDAD_KEY
        3. CONCEPTO_KEY (flexible)
        4. POLIZA_KEY (solo si no es NULL)
        5. IMPORTE_KEY
        
        Returns:
            Tuple[costos_clasificados, contabilidad_clasificada, detalle_matches]
        """
    ...
    def _buscar_mejor_match_base(self, row_base: pd.Series, contabilidad: pd.DataFrame) -> Dict:
        ...
        # Filtros progresivos para reducir espacio de búsqueda
        candidatos = contabilidad.copy()
        
        # Filtro 1: POLIZA (más restrictivo)
    ...
    def _buscar_mejor_match_costo(self, row_costo: pd.Series, contabilidad: pd.DataFrame) -> Dict:
        ...
        for _, row_cont in candidatos.iterrows():
            ...
            # 4. POLIZA/CONTRARECIBO (solo si ambos tienen valor)
    ...
    def _clasificar_contabilidad_inversa(self, 
        ...
        col_id_origen = 'ROW_ID_BASE' if origen == 'BASE' else 'ROW_ID_COSTO'
...
class DeduplicadorCostos:
    ...
    def deduplicate(self, 
        ...
        - VALE_KEY
        - UNIDAD_KEY
        - CONCEPTO_KEY (similar)
        - IMPORTE_KEY (diferencia < 0.01)
        
        Returns:
            Tuple[costos_depurados, duplicados_detectados]
        """
        # Combinar ambas fuentes
        cheques_work = cheques.copy()
        vouchers_work = vouchers.copy()
        
        # Asegurar que tienen las mismas columnas clave
        columnas_clave = ['VALE_KEY', 'UNIDAD_KEY', 'CONCEPTO_KEY', 'IMPORTE_KEY', 'POLIZA_KEY', 'ORIGEN']
        ...
        # Agrupar por VALE para reducir comparaciones
        ...
        for vale, grupo in grupos_vale:
            ...
            for i, row1 in grupo.iterrows():
                ...
                for j, row2 in grupo.iterrows():
                    ...
                    if es_duplicado:
                        # Marcar el segundo como duplicado
        ...
        # Separar depurados y duplicados
    ...
    def _son_duplicados(self, row1: pd.Series, row2: pd.Series) -> bool:
        ...
        # 1. VALE (debe coincidir)
        if row1['VALE_KEY'] and row1['VALE_KEY'] == row2['VALE_KEY']:
            criterios_cumplidos += 1
        else:
            return False  # Vale es obligatorio
        ...
        # Considerar duplicado si cumple 3+ criterios
...
class AnalizadorDH:
    ...
    def calcular_saldos(contabilidad_completa: pd.DataFrame) -> pd.DataFrame:
        ...
        Agrupa por: POLIZA_KEY, UNIDAD_KEY, CONCEPTO_KEY
        Calcula: TOTAL_D, TOTAL_H, SALDO, ESTATUS
        """
...
# ============================================================
# GENERADOR DE SALDOS FINALES
# ============================================================

class GeneradorSaldosOwner:
    """Genera saldos finales consolidados por Owner"""
    
    def __init__(self, config: ConfigConciliacion):
        self.config = config
    
    def generar_saldos(self,
                      base_clasificada: pd.DataFrame,
                      costos_clasificados: pd.DataFrame,
                      contabilidad_completa: pd.DataFrame) -> pd.DataFrame:
        """
        Genera reporte final de saldos por Owner
        
        Estructura:
        - OWNER
        - TOTAL_INGRESOS (liquidaciones)
        - TOTAL_COSTOS (base + cheques/vouchers)
        - SALDO_NETO
        - DETALLE_MATCH_OK
        - DETALLE_CON_DISCREPANCIA
        - DETALLE_NO_MATCH
        """
        
        # TODO: Esta función requiere que los datos tengan columna de OWNER
        # Por ahora retornamos estructura básica
        
        st.warning("⚠️ Generación de saldos por Owner pendiente de implementar según estructura final de datos")
...
def main():
    st.set_page_config(page_title="Sistema de Conciliación Owner v3.0", layout="wide")
    
    st.title("🔄 Sistema Unificado de Conciliación de Saldos Owner")
    st.caption("Versión 3.0 - Optimizada, Modular y Auditable")
    ...
    with st.expander("ℹ️ Información del Sistema", expanded=False):
        ...
        1. **Matching Inteligente**
           ...
           - Manejo robusto de conceptos en inglés/español
           - Detección de similitudes semánticas
        ...
        3. **Deduplicación Avanzada**
           - Detección automática de duplicados entre Cheques y Vouchers
           - Criterios configurables de duplicidad
        
        4. **Análisis D vs H**
           - Identificación de saldos pendientes vs liquidados
           - Agrupación por póliza, unidad y concepto
        
        5. **Trazabilidad Completa**
           - Detalle de coincidencias por criterio
           - Score normalizado para cada match
           - Auditoría de exclusiones y duplicados
        """)
    ...
    try:
        ...
        st.success(f"✅ Contabilidad D: {len(cont_d):,} registros | Total: {len(cont_completa):,}")
        
        resultados['Contabilidad_D'] = cont_d
        resultados['Contabilidad_Completa'] = cont_completa
        
        # ============================================================
        # PASO 2: Módulo BASE SALDOS
        # ============================================================
        if base_file:
            st.header("📋 2. Procesando Base Saldos")
        ...
        # ============================================================
        # PASO 3: Módulo CHEQUES Y VOUCHERS
        # ============================================================
        if cheques_file and vouchers_file:
            st.header("💵 3. Procesando Cheques y Vouchers")
        ...
        # ============================================================
        # PASO 4: Análisis D vs H
        # ============================================================
        st.header("⚖️ 4. Análisis de Saldos D vs H")
